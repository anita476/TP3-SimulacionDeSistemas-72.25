...
- **⟨t90⟩:** semillas 601 a 612, las de confirmación del 1.2. Mesa vacía y configuración elegida: esos valores. Disco grande y embudo: medidos en esas mismas semillas. Las barras son el desvío estándar muestral.
...
D es de una sola realización, así que no tiene desvío. La elegida es el disco de R = 0.34 m centrado en (0.59 m, 0.34 m), `configs/ga_best.txt`. El disco grande es un obstáculo en (0.60 m, 0.34 m) con R = 0.22 m (`configs/disco_grande.txt`). El embudo son dos discos de R = 0.075 m en x = 0.45 m, y = 0.145 m y y = 0.535 m (`configs/embudo.txt`).
...
- La elegida es el mismo tipo de disco central, con radio 0.34 m: ocupa todo el ancho de la mesa.
...
- La elegida tiene el menor D (0.00465 m²/s) y el menor ⟨t90⟩ (15.61 s). El disco grande queda en el medio en las dos (0.0122 m²/s y 19.72 s).
- El embudo tiene más difusión que la mesa vacía (0.0188 contra 0.0169 m²/s) y un ⟨t90⟩ menor (21.70 s contra 23.76 s).
- Entre la elegida y la mesa vacía las barras no se solapan: 8.15 s de diferencia de medias, con desvíos de 2.47 s y 3.22 s. El disco y el embudo sí se solapan con la mesa vacía.
...
- La elegida reúne el menor D y el menor ⟨t90⟩, y el disco grande queda en el medio. El embudo no sigue esa orden: difunde más que la mesa vacía y aun así llega un poco antes. El D chico de la elegida sale de que el disco tapa el ancho entero y el desplazamiento en y se satura enseguida.
