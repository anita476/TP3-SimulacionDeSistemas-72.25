...
void print_summary(const Simulation &sim, const SimParams &params,
                   const GeneratorConfig &gen, const GeneratorStats &gen_stats,
                   const std::string &out_path, double initial_energy,
                   double final_energy, double init_seconds,
                   double loop_seconds) {
  ...
  std::cout << "\nSimulation results\n"
            ...
            << "  Particles that scored:    " << sim.goals() << " / " << gen.N
            << std::setprecision(1) << " (" << 100.0 * sim.goals() / gen.N
            << "%)\n";
  ...
  // } else {
  //     std::cout << "  Relative energy drift:    undefined (initial energy is
  //     zero)\n"
...
int main(int argc, char *argv[]) {
  argparse::ArgumentParser program("EventDriven-TP3", "0.2",
  ...
  program.add_argument("-W").default_value(0.68).scan<'g', double>().help(
      "table width (m)");
  program.add_argument("-d").default_value(0.20).scan<'g', double>().help(
      "goal width, centred on each short wall (m)");

  program.add_argument("-N").default_value(100).scan<'i', int>().help(
      "particle count (default 100)");
  program.add_argument("-r").default_value(0.0175).scan<'g', double>().help(
      "particle radius (m)");
  program.add_argument("-m").default_value(0.025).scan<'g', double>().help(
      "particle mass (kg)");
  program.add_argument("-v0").default_value(1.0).scan<'g', double>().help(
      "initial speed (m/s)");

  program.add_argument("-tmax").default_value(30.0).scan<'g', double>().help(
      "maximum simulated time (s) (default 30)");
  program.add_argument("-k").default_value(200).scan<'i', int>().help(
      "dump every k physical events (0 = only at goals) (default 200)");
  program.add_argument("-seed").default_value(0).scan<'i', int>().help(
      "RNG seed (0 = random / no fixed seed)");
  ...
  program.add_argument("--raw").default_value(false).implicit_value(true).help(
      "print key-value summary for scripts");
  ...
  try {
    ...
    // init = construccion (grilla, indice de obstaculos, predicciones
    // iniciales)
    ...
    if (program.get<bool>("--raw")) {
      ...
      std::cout << std::defaultfloat << std::setprecision(17) << "N " << N
                << '\n'
                << "K " << obstacles.size() << '\n'
                << "seed " << seed << '\n'
                << "tmax " << tmax << '\n'
      ...
      if (e0 > 0.0) {
        std::cout << "energy_drift " << std::fabs(e_end - e0) / e0 << '\n';
      } else {
      ...
      print_summary(sim, params, gen, gen_stats, out_path, e0, e_end,
                    init_seconds, loop_seconds);
    }
