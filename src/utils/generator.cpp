...
void validate(const GeneratorConfig &cfg) {
  if (cfg.N < 1)
    throw std::invalid_argument("N debe ser >= 1");
  if (cfg.L <= 0.0 || cfg.W <= 0.0)
    throw std::invalid_argument("L y W deben ser positivos");
  if (cfg.r <= 0.0 || cfg.m <= 0.0)
    throw std::invalid_argument("r y m deben ser positivos");
  if (cfg.max_attempts < 1)
    throw std::invalid_argument("max_attempts debe ser >= 1");
  if (cfg.L <= 2.0 * cfg.r || cfg.W <= 2.0 * cfg.r)
    throw std::invalid_argument(
        "la mesa es más chica que el diámetro de la partícula");
}
...
bool overlaps_placed(double x, double y, double r,
  ...
  for (int dy = -1; dy <= 1; ++dy) {
    for (int dx = -1; dx <= 1; ++dx) {
      ...
      if (nx < 0 || nx >= grid.side() || ny < 0 || ny >= grid.width())
        continue;
      for (int j : grid.cell(grid.cell_index(nx, ny))) {
        if (discs_overlap(x - placed[j].x, y - placed[j].y, r + placed[j].r))
          return true;
      }
    }
  }
...
bool overlaps_obstacle(double x, double y, double r,
                       const std::vector<Obstacle> &obstacles) {
  for (const Obstacle &o : obstacles)
    if (discs_overlap(x - o.kx, y - o.ky, r + o.Rk))
      return true;
  return false;
}
...
std::vector<Particle> generate_particles(const GeneratorConfig &cfg,
  ...
  for (int i = 0; i < cfg.N; ++i) {
    ...
    for (int attempt = 0; attempt < cfg.max_attempts && !placed; ++attempt) {
      ...
      if (overlaps_obstacle(x, y, cfg.r, cfg.obstacles) ||
          overlaps_placed(x, y, cfg.r, particles, grid))
        continue;

      grid.insert(i, x, y);

      const double theta = angle(rng);
      particles.push_back(Particle{x, y, cfg.r, cfg.v0 * std::cos(theta),
...
int find_overlap(const std::vector<Particle> &particles,
                 const std::vector<Obstacle> &obstacles, double L, double W) {
  const int n = static_cast<int>(particles.size());

  for (int i = 0; i < n; ++i) {
    const Particle &a = particles[i];
    if (!disc_inside(a.x, a.y, a.r, L, W))
      return i;
    for (const Obstacle &o : obstacles)
      if (discs_overlap(a.x - o.kx, a.y - o.ky, a.r + o.Rk))
        return i;
    for (int j = i + 1; j < n; ++j)
      if (discs_overlap(a.x - particles[j].x, a.y - particles[j].y,
                        a.r + particles[j].r))
        return i;
  }
  return -1;
